...
export default function Leads() {
  ...
  // Filtro inteligente e Injeção de AI Score em tempo real
  const filteredLeads = useMemo(() => {
    return leads
      ...
      .sort((a, b) => b.aiScore - a.aiScore); // Ordena pelos mais quentes primeiro
  }, [leads, searchTerm]);
  ...
  return (
    <div className="space-y-8 pb-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-3xl font-black text-muted-900 dark:text-white tracking-tight italic uppercase">Lead Intelligence</h1>
          <p className="text-muted-500 dark:text-muted-400 font-medium">Predictive scoring for high-ticket opportunities.</p>
        </div>
      ...
      <div className="flex gap-3">
        <div className="flex-1">
          <Input 
            icon={Search} 
            placeholder="Search by intelligence..." 
      ...
      <Card className="overflow-hidden border-none shadow-2xl shadow-black/5" animate={false}>
        {loading ? (
          <div className="p-20 flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-[10px] font-black text-muted-400 uppercase tracking-[0.3em]">Processing Neural Data...</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-muted-50/50 dark:bg-muted-900/50 border-b border-muted-100 dark:border-muted-800">
                  ...
                  <th className="px-6 py-4 text-right text-[10px] font-black uppercase tracking-[0.2em] text-muted-400">Action</th>
                </tr>
              </thead>
                    ...
                    >
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-xl bg-muted-100 dark:bg-muted-800 text-muted-700 dark:text-muted-200 flex items-center justify-center font-black text-xs">
                            {lead.name?.charAt(0)}
                          </div>
                          <div className="flex flex-col">
                            ...
                            <span className="text-[11px] text-muted-500 font-medium">{lead.email || 'No email recorded'}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex flex-col items-center gap-1">
                          ...
                          <div className="w-16 h-1 bg-muted-100 dark:bg-muted-800 rounded-full overflow-hidden">
                            <div className="h-full bg-ai-500 transition-all duration-1000" style={{ width: `${lead.aiScore}%` }} />
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-right">
                        <button onClick={() => handleDelete(lead.id)} className="p-2 text-muted-300 hover:text-rose-500 transition-colors">
                          <Trash2 size={16} />
                        </button>
                      </td>
                    </motion.tr>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Capture Lead">
        <form onSubmit={handleCreateLead} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input label="Name" icon={User} required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-black uppercase text-muted-400">Status</label>
              <select 
                className="w-full h-11 bg-muted-50 dark:bg-muted-900 border border-muted-200 dark:border-muted-800 rounded-xl px-4 text-sm font-bold outline-none focus:ring-2 focus:ring-primary-500" 
          ...
          <div className="flex gap-3 pt-4">
            ...
            <Button type="submit" variant="primary" className="flex-1" isLoading={isSubmitting}>Capture</Button>
          </div>
        </form>
      </Modal>
    </div>
